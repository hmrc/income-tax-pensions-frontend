...
class ReliefAtSourcePaymentsAndTaxReliefAmountController @Inject() (authAction: AuthorisedAction,
  ...
  def submit(taxYear: Int): Action[AnyContent] = authAction.async { implicit request =>
    formProvider.reliefAtSourcePaymentsAndTaxReliefAmountForm
      .bindFromRequest()
      .fold(
          ...
          pensionSessionService.getPensionsSessionDataResult(taxYear, request.user) { optData =>
            ...
            redirectBasedOnCurrentAnswers(taxYear, optData, cyaPageCall(taxYear))(checkRedirect) { data =>
              ...
              pensionSessionService.createOrUpdateSessionData(request.user, updatedCyaModel, taxYear, data.isPriorSubmission)(
                ...
                isFinishedCheck(updatedCyaModel.paymentsIntoPension, taxYear, ReliefAtSourceOneOffPaymentsController.show(taxYear), cyaPageCall)
              }
            }
          }
      )
