...
class StatePensionLumpSumControllerISpec extends IntegrationTest with BeforeAndAfterEach with ViewHelpers with PensionsDatabaseHelper {
    ...
    "Return OK response 'Did you get a State Pension lump sum?' page with correct content and no pre-filling" in {
      implicit lazy val result: WSResponse = {
        ...
        val pensionsViewModel =
          anIncomeFromPensionsViewModel.copy(statePensionLumpSum = Some(anStateBenefitViewModelOne.copy(amountPaidQuestion = None)))
  ...
  ".submit" should {
    ...
    "redirect to the 'Tax paid on state pension lump sum' page when submitting answer as 'Yes' with correct taxPaid amount" ignore {
      ...
      cyaModel.pensions.incomeFromPensions.statePensionLumpSum shouldBe Some(
