...
abstract class BaseYesNoController(
  ...
  def whenFormIsInvalid(form: Form[Boolean], taxYear: Int)
                       (implicit request: AuthorisationRequest[AnyContent]): Html
  ...
  private def show(pensionsUserData: PensionsUserData, taxYear: Int)
  ...
  private def onValidForm(pensionsUserData: PensionsUserData, taxYear: Int, validForm: Boolean)
