...
class RemoveTransferChargeViewSpec extends ViewUnitTest {
  ...
  private lazy val underTest = inject[RemoveTransferChargeSchemeView]
  "on show" should {
    userScenarios.foreach {
      userScenario =>
        ...
        s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" which {
          "render the remove transfer charge pension scheme page" which {


            val htmlFormat = underTest(taxYearEOY, pensionName, Some(1))
            implicit val document: Document = Jsoup.parse(htmlFormat.body)

            val url = controllers.pensions.transferIntoOverseasPensions.routes.TransferChargeSummaryController.show(taxYearEOY).url
