...
object PaymentsIntoOverseasPensionsAudit {
  ...
  def apply(taxYear: Int,
            user: User,
            paymentsIntoOverseasPensions: PaymentsIntoOverseasPensionsViewModel,
            priorPaymentsIntoOverseasPensions: Option[PaymentsIntoOverseasPensionsViewModel]): PaymentsIntoOverseasPensionsAudit =
    PaymentsIntoOverseasPensionsAudit(
      ...
      priorPaymentsIntoOverseasPensions
    )
  ...
  def amendAudit(user: User, sessionData: PensionsUserData, priorData: Option[AllPensionsData]): PaymentsIntoOverseasPensionsAudit =
    PaymentsIntoOverseasPensionsAudit(
      ...
      priorPaymentsIntoOverseasPensions = priorData.map(pd => AllPensionsData.generateSessionModelFromPrior(pd).paymentsIntoOverseasPensions)
    )
