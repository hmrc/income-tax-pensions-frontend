          ...
          case Right(Some(data)) =>
            ...
            if (validateCountryIndex(index, pensionSchemeList)) {
              val updatedCyaModel = index match {
                case Some(value) =>
                  val scheme = pensionSchemeList(value).copy(countryCode2d = Some(country))
                  data.pensions.copy(
                ...
                case None =>
                  ...
                  data.pensions.copy(
                    incomeFromOverseasPensions = data.pensions.incomeFromOverseasPensions.copy(
                      overseasIncomePensionSchemes = currentSchemes ++ Seq(PensionScheme(countryCode2d = Some(country)))
                    ))
              }
