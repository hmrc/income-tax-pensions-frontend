...
object PaymentsIntoPensionsRedirects { // scalastyle:off magic.number
  ...
  private val pageValidInJourneyMap: Map[Int, PaymentsIntoPensionsViewModel => Boolean] = {
    ...
    val taxReliefNotClaimedQuestionFn = { pIPViewModel: PaymentsIntoPensionsViewModel =>
      pIPViewModel.pensionTaxReliefNotClaimedQuestion.getOrElse(false)
    }

    Map(
      // ^ 2,3,5 need Q1 ^
      2 -> rasPaymentQuestionFn,
      3 -> rasPaymentQuestionFn,
      5 -> rasPaymentQuestionFn,
      ...
      // ^ 7,9 need Q6 ^
      7 -> taxReliefNotClaimedQuestionFn,
      9 -> taxReliefNotClaimedQuestionFn,
