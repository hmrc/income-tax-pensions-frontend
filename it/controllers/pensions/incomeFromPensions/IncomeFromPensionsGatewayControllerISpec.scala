...
class IncomeFromPensionsGatewayControllerISpec extends IntegrationTest
  with ViewHelpers
  ...
  private def url(taxYear: Int): String = {
    s"/update-and-submit-income-tax-return/pensions/$taxYear/pension-income/income-from-pensions"
  }
  ...
  ".submit" should {
    ...
    "return a bad request when tailoring is enabled and 'form' is invalid" in {
      val request = FakeRequest("POST", url(taxYearEOY)).withHeaders(HeaderNames.COOKIE -> playSessionCookies(taxYearEOY, validTaxYearList), csrfContent)
