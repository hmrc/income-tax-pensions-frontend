...
final case class ConnectorResponseInfo(method: String, url: String, response: HttpResponse) {
  private def logMessage: String = {
    ...
    s"Connector [$CorrelationIdHeaderKey=${response.correlationId.getOrElse(
        "Unknown")}]: Response Received for $method $url. Response status: ${response.status}" + nonSuccessBody
  }
