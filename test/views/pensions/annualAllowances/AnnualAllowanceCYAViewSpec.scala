...
class AnnualAllowanceCYAViewSpec extends ViewUnitTest {
  ...
  userScenarios.foreach { userScenario =>
    ...
    val booleanToYesOrNo: Option[Boolean] => String = bool =>
      if (bool.getOrElse(false)) userScenario.commonExpectedResults.yesText else userScenario.commonExpectedResults.noText
    ...
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {  //scalastyle:off magic.number
      ...
      "render the page with a full CYA model" which {
        ...
        cyaRowCheck(reducedAnnualAllowance, booleanToYesOrNo(viewModel.reducedAnnualAllowanceQuestion),
          ChangeLinks.changeReducedAnnualAllowance, reducedAnnualAllowanceHidden, 1)

        cyaRowCheck(typeOfReducedAnnualAllowance, viewModel.typeOfAllowance.getOrElse(Nil).mkString(", "),
          ChangeLinks.changeTypeOfReducedAnnualAllowance, typeOfReducedAnnualAllowanceHidden, 2)

        cyaRowCheck(aboveAnnualAllowance, booleanToYesOrNo(viewModel.aboveAnnualAllowanceQuestion),
          ChangeLinks.changeAboveAnnualAllowance, aboveAnnualAllowanceHidden, 3)

        cyaRowCheck(amountAboveAnnualAllowance, moneyContent(viewModel.aboveAnnualAllowance.getOrElse(0)),
          ChangeLinks.changeAboveAnnualAllowance, aboveAnnualAllowanceHidden, 4)

        cyaRowCheck(annualAllowanceTax, moneyContent(viewModel.taxPaidByPensionProvider.getOrElse(0)),
          ChangeLinks.changeAnnualAllowanceTax, annualAllowanceTaxHidden, 5)

        cyaRowCheck(annualAllowanceSchemes, s"${viewModel.pensionSchemeTaxReferences.getOrElse(Nil)
          .mkString(", ")}",
          ChangeLinks.changeAnnualAllowanceSchemes, annualAllowanceSchemesHidden, 6)
