...
class AuditActionsProvider @Inject() (authAction: AuthorisedAction,
  ...
  def paymentsIntoOverseasPensionsViewAuditing(taxYear: TaxYear): ActionBuilder[UserSessionDataRequest, AnyContent] =
    authoriseWithSessionAndPrior(taxYear, Journey.PaymentsIntoOverseasPensions)
  ...
  def transfersIntoOverseasPensionsViewAuditing(taxYear: TaxYear): ActionBuilder[UserSessionDataRequest, AnyContent] =
    authoriseWithSessionAndPrior(taxYear, Journey.TransferIntoOverseasPensions)
