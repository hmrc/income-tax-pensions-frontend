...
class PensionSchemeSummaryController @Inject() (actionsProvider: ActionsProvider, pageView: PensionsSchemeSummary, cc: MessagesControllerComponents)(
  ...
  def show(taxYear: Int, index: Option[Int]): Action[AnyContent] = actionsProvider.authoriseWithSession(taxYear) async { implicit sessionUserData =>
    indexCheckThenJourneyCheck(sessionUserData.sessionData, index, PensionSchemeSummaryPage, taxYear) { data =>
  ...
  def submit(taxYear: Int, index: Option[Int]): Action[AnyContent] = actionsProvider.authoriseWithSession(taxYear) async { implicit sessionUserData =>
    indexCheckThenJourneyCheck(sessionUserData.sessionData, index, PensionSchemeSummaryPage, taxYear) { _ =>
