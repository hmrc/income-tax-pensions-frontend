...
trait CYABaseHelper {
  def summaryListRow(labelMessageKey: String, displayedValue: String, changeLink: Call)(implicit messages: Messages): SummaryListRow =
    ViewUtils.summaryListRow(
      ...
      actions = Seq((changeLink, messages("common.change"), Some(messages(labelMessageKey + ".hidden"))))
    )

  def summaryListRowWithBooleanValue(labelMessageKey: String, valueOpt: Option[Boolean], changeLink: Call, suffixOpt: Option[String] = None)(implicit
      messages: Messages): SummaryListRow =
    summaryListRow(labelMessageKey, displayedValue(valueOpt, suffixOpt), changeLink)

  def summaryListRowWithOptionalAmountValue(labelMessageKey: String, value: Option[BigDecimal], changeLink: Call)(implicit
      messages: Messages): SummaryListRow =
  ...
  def summaryListRowWithStrings(labelMessageKey: String, valueOpt: Option[Seq[String]], changeLink: Call)(implicit
      messages: Messages): SummaryListRow =
