  ...
  userScenarios.foreach { userScenario =>
    s"language is ${welshTest(userScenario.isWelsh)} and request is from an ${agentTest(userScenario.isAgent)}" should {
      "Render the page with prefilled data for Migrant Scheme" which {
        ...
        textOnPageCheck(
          userScenario.commonExpectedResults.schemeName + " " + reliefToRemove.customerReference.getOrElse(""),
          Selectors.reliefSelector(1))
        textOnPageCheck(
          userScenario.commonExpectedResults.untaxedEmployerPayments + " " + reliefToRemove.employerPaymentsAmount.getOrElse(""),
          Selectors.reliefSelector(2))
