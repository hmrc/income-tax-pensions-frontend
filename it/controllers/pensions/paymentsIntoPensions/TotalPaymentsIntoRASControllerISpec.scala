  ...
  ".show" should {
    "render 'Total payments into RAS pensions' page with correct content and no pre-filling" which {
      lazy val result: WSResponse = {
        ...
        urlGet(
          fullUrl(totalPaymentsIntoRASUrl(taxYearEOY)),
          follow = false,
    ...
    "render 'Total payments into RAS pensions' page without a one-off amount" should {
      lazy val result: WSResponse = {
        ...
        dropPensionsDB()
        insertCyaData(
          pensionsUsersData(requiredViewModel.copy(oneOffRasPaymentPlusTaxReliefQuestion = Some(false), totalOneOffRasPaymentPlusTaxRelief = None)))
        urlGet(
          fullUrl(totalPaymentsIntoRASUrl(taxYearEOY)),
          follow = false,
          headers = Seq(HeaderNames.COOKIE -> playSessionCookies(taxYearEOY, validTaxYearList)))
