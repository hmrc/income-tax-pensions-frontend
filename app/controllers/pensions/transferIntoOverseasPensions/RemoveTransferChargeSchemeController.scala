...
class RemoveTransferChargeSchemeController @Inject()(actionsProvider: ActionsProvider,
  ...
  def show(taxYear: Int, index: Option[Int]): Action[AnyContent] = actionsProvider.userSessionDataFor(taxYear) async { implicit sessionUserData =>
    ...
    validateIndex(index, transferChargeScheme.size).fold(Future.successful(Redirect(routes.TransferChargeSummaryController.show(taxYear)))) {
      i =>
        transferChargeScheme(i).name.fold(Future.successful(Redirect(routes.TransferChargeSummaryController.show(taxYear)))){
